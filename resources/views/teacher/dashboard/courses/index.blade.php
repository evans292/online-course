            ...
            <div class="p-6 bg-white border-b border-gray-200">
                <div class="bg-white pb-4 px-4 rounded-md w-full">
                    <div class="flex justify-between w-full pt-6 ">
                      <h1 class="p-1 text-xl font-semibold">Courses Table</h1>
                    ...
                    <a href="{{ route('teacher.course.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-400 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150"><i class="fas fa-plus mr-1"></i>Add course</a>
                    </div>
                    ...
                    <table class="table-auto border-collapse w-full">
                      <thead>
                        <tr class="rounded-lg text-sm font-medium text-gray-700 text-left" style="font-size: 0.9674rem">
                          ...
                          <th class="px-4 py-2 " style="background-color:#f8f8f8">Information</th>
                          <th class="px-4 py-2 " style="background-color:#f8f8f8">Total Subject</th>
                          <th class="px-4 py-2 " style="background-color:#f8f8f8">Total Class</th>
                      ...
                      <tbody class="text-sm font-normal text-gray-700">
                        ...
                        <tr class="hover:bg-gray-100 border-b border-gray-200 py-10">
                          ...
                          <td class="px-4 py-4">{!! ($data->subjectmatters->count() !== null) ? $data->subjectmatters->count() : '<span class="text-gray-400">NULL</span>'!!}</td>
                          <td class="px-4 py-4">{!! ($data->schoolclasses->count() !== null) ? $data->schoolclasses->count() : '<span class="text-gray-400">NULL</span>'!!}</td>
                          <td class="px-4 py-4">{!! ($data->teachers->count() !== null) ? $data->teachers->count()  : '<span class="text-gray-400">NULL</span>'!!}</td>
                          {{-- <td class="px-4 py-4">{{ $data->created_at }}</td>  --}}
                          <td class="px-4 py-4">
                            <form id="{{ $data->id }}" action="{{ route('teacher.course.destroy', ['course' => $data->id]) }}" method="POST">
                              @csrf
                              @method('delete')
                            </form>
                            <a href="#" onclick="deleteConfirm('{{ $data->name }}', '{{ $data->id }}')"><i class="fas fa-trash-alt text-red-400 mr-1"></i></a>
                            <a href="{{ route('teacher.course.edit', ['course' => $data->id]) }}"><i class="fas fa-pencil-alt text-yellow-400"></i></a>
                          </td>
                        </tr>
                        @endforeach
                      </tbody>

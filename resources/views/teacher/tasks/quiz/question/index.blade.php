        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
              ...
              <div class="flex justify-between">
                ...
                <a href="{{ route('teacher.question.create', ['class' => Request::segment(3), 'quiz' => Request::segment(4)]) }}" class="inline-flex items-center px-4 py-2 bg-indigo-400 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:ring ring-gray-300 disabled:opacity-25 transition ease-in-out duration-150"><i class="fas fa-plus mr-1"></i>Add question</a>
              </div>
              @else
                        ...
                        <table class="table-auto border-collapse w-full">
                          ...
                          <tbody class="text-sm font-normal text-gray-700">
                            ...
                            <tr class="hover:bg-gray-100 border-b border-gray-200 py-10">
                              ...
                              <td class="px-4 py-4"><a class="text-green-400 hover:text-green-600" href="{{ route('teacher.question.show', ['class' => Request::segment(3), 'quiz' => $data->quiz_id, 'question' => $data->id]) }}">{!! Str::limit($data->question, 50)  !!}</a></td>
                              <td class="px-4 py-4">{{ $data->created_at->diffForHumans() }}</td>
                              <td class="px-4 py-4">
                                <form id="{{ $data->id }}" action="{{ route('teacher.question.destroy', ['class' => Request::segment(3), 'quiz' => $data->quiz_id, 'question' => $data->id]) }}" method="POST">
                                  @csrf
                                  @method('delete')
                                </form>
                                <a href="#" onclick="deleteConfirm('{{ $data->question }}', '{{ $data->id }}')"><i class="fas fa-trash-alt text-red-400 mr-1"></i></a>
                                <a href="{{ route('teacher.question.edit', ['class' => Request::segment(3), 'quiz' => $data->quiz_id, 'question' => $data->id]) }}"><i class="fas fa-pencil-alt text-yellow-400"></i></a>
                              </td>
                            </tr>
                            @endforeach
                          </tbody>

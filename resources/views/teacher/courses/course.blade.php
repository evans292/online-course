            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 bg-white border-b border-gray-200">
                        ...
                        <ul class="list-reset flex flex-col">
                            ...
                            @if (Auth::user()->teachers[0]->courses->count() === 0)
                                <h5 class="text-gray-400 p-2">contact admin to map your subject!</h5>
                            @else
                            @foreach ($courses as $course)
                                <li class="rounded-t relative -mb-px block border p-4 border-grey"><a href="{{ route('teacher.subjectmatters', ['courses' => $course->id]) }}" class="hover:text-green-400">{{ $course->name }}</a></li>
                            @endforeach
                            @endif
                        </ul>
                    {{-- @endif --}}
